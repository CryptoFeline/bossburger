...
const CanvasEditor: React.FC<CanvasEditorProps> = ({
  ...
  useEffect(() => {
    ...
    if (appState === 'FINAL') {
      // Example: Draw watermark at bottom right of the image
      const watermarkWidth = 80; // or your watermark image width
      const watermarkHeight = 30; // or your watermark image height
      const margin = 12; // margin from edge

      // Calculate position relative to the image, not the canvas
      const x = offsetX + imgW - watermarkWidth - margin;
      const y = offsetY + imgH - watermarkHeight - margin;

      // If Watermark is an image:
      // ctx.drawImage(watermarkImage, x, y, watermarkWidth, watermarkHeight);

      // If Watermark is a function/component:
      Watermark.draw(ctx, x, y, watermarkWidth, watermarkHeight);
    }
