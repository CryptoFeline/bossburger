...
const CanvasEditor: React.FC<CanvasEditorProps> = ({
  ...
  useEffect(() => {
    ...
    if (appState === 'FINAL' && editorImage) {
      // Calculate scaled image size and offset
      const scale = Math.min(
        canvasSize / editorImage.width,
        canvasSize / editorImage.height
      );
      ...
      // Watermark size (max 1/4 image width)
      const maxWatermarkWidth = imgW / 4;
      const watermarkImg = new window.Image();
      watermarkImg.src = '/BossBurger.svg';
      watermarkImg.onload = () => {
        const scaleW = Math.min(1, maxWatermarkWidth / watermarkImg.width);
        const w = watermarkImg.width * scaleW;
        const h = watermarkImg.height * scaleW;
        const margin = 16;
        const x = offsetX + imgW - w - margin;
        const y = offsetY + imgH - h - margin;
        ctx.globalAlpha = 0.7;
